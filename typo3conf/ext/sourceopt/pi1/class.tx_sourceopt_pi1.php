...
class tx_sourceopt_pi1 extends tslib_pibase {
	...
	function init(&$content, $conf) {
		...
		switch (TYPO3_OS) {  // set newline
			case "WIN" : $this->newline = "\r\n";
				break;
			default : $this->newline = "\n";
		}
		$this->tab = "\t";
		if ($this->conf['formatHtml.']['tabSize'] && is_numeric($this->conf['formatHtml.']['tabSize'])) {
	...
	function includeHeaderComment(&$html) {
		$html = preg_replace_callback('/<meta http-equiv(.*)>/Usi', array($this, 'includeHeaderComment_callback'), $html);
	}
	...
	function formatHtml(&$html) {
		...
		/*
		  ...
		  This part was replaced by Christoph Dörfel
		...
		for ($x = 0; $x < count($html_array); $x++) {
			...
			elseif ($this->conf['formatHtml'] >= 4 && (  // logical line break
				   # this element has a line break before itself
				   preg_match('/<' . $allBoxLikeElements . '(.*)>/Usi', $html_array[$x]) ||
				   preg_match('/<' . $allBoxLikeElements . '(.*) \/>/Usi', $html_array[$x]) ||
				   # one element before is a element that has a line break after
			...
			if (substr($html_array[$x - 1], 0, 4) == '<pre'  // remove white space after line ending in PRE / TEXTAREA / comment
				   || substr($html_array[$x - 1], 0, 9) == '<textarea'
				   || substr($html_array[$x - 1], 0, 4) == '<!--') {
			...
			if (substr($html_array[$x], 0, 1) == '<' && substr($html_array[$x], 1, 1) != '/') {
				if (substr($html_array[$x], 1, 1) != ' ' &&
					   substr($html_array[$x], 1, 3) != 'img' &&
					   substr($html_array[$x], 1, 2) != 'br' &&
					   substr($html_array[$x], 1, 2) != 'hr' &&
					   substr($html_array[$x], 1, 5) != 'input' &&
					   substr($html_array[$x], 1, 4) != 'link' &&
					   substr($html_array[$x], 1, 4) != 'meta' &&
					   substr($html_array[$x], 1, 4) != 'col ' &&
					   substr($html_array[$x], 1, 5) != 'frame' &&
					   substr($html_array[$x], 1, 7) != 'isindex' &&
					   substr($html_array[$x], 1, 5) != 'param' &&
					   substr($html_array[$x], 1, 4) != 'area' &&
					   substr($html_array[$x], 1, 4) != 'base' &&
	...
	function trimLines(&$html) {
		...
		if (TYPO3_OS == "WIN")
			...
			$html = str_replace("\r\n", "\n", $html);
		$temp = explode($this->newline, $html);
		array_walk($temp, array($this, 'trimLines_callback'));
		$html = implode($this->newline, $temp);
		unset($temp);
	}
	...
	function removeNewLines(&$html) {
		$splitArray = array('textarea', 'pre'); // eventuell auch: span, script, style
